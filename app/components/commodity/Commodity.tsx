...
export default function Commodity() {
    ...
    return (
        <div id="products" className="flex flex-col justify-center mt-8 mb-6 text-[#4166e0]">
            ...
            <motion.section
                className="flex flex-col justify-center mt-8 p-4"
                initial={{ opacity: 0, x: 50 }}
                whileInView={{ opacity: 1, x: 0 }}
                transition={{ duration: 2 }}
                viewport={{ once: true }}
            >
                <Carousel className="w-full max-w-5xl mx-auto">
                    <CarouselContent className="-ml-1 flex my-8 space-x-4">
                        {products.sort(() => Math.random() - 0.5).slice(0, 9).map((pro) => (
                            <CarouselItem key={pro.id} className="flex justify-center items-center w-full sm:basis-1/2 md:basis-1/3 ">
                                <ProductCard
